...
- обновляется вручную по кнопке ↻ и автоматически по таймеру (по умолчанию раз в 6 часов);
- пишет журнал инициализации и результатов автообновления в `state.subscriptionAutoRefresh.log` (время запуска, итог по каждой подписке, ошибки/пропуски);
